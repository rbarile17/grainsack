...
from grainsack.explain import run_explanation_task
...
def explain(kg: str, model: str, method: str, mode: str) -> None:
    set_seeds(42)
    run_explanation_task(kg, model, method, mode)
...
def predict(kg: str, model: str) -> None:
    ...
    evaluator.evaluate(
        ...
        mapped_triples,
        batch_size=64,
        additional_filter_triples=[kg.training.mapped_triples, kg.validation.mapped_triples],
    )
...
def dixit(kg, model, method, mode, parameters, llm):
    set_seeds(42)

    run_dixit_task(kg, model, method, mode, parameters, llm)
