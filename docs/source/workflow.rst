The Benchmarking Activities and Workflow
========================================
...
To achieve this, GRainsaCK defines a set of activities to be performed in benchmarking LP-X methods, each of them formalized and implemented as a function.
In addition to the entire workflow, GRainsaCK also allows executing each activity on its own either via CLI or API.
In the following, we describe and formalize the signature of each function (activity), i.e., the formal interface that defines the input/output types.
Let 
