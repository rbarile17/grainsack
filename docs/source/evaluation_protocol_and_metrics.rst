...
Specifically, four alternative prompting methods are considered: zero-shot (task input only) without output constraints, few-shot (task input and examples) without output constraint, zero-shot with output constraints, and few-shot with output constraints.
LP-DIXIT defines a general prompt template that is instantiated according to the selected prompting method and the explanation to be evaluated, as illustrated in the following (Prompt template used by LP-DIXIT with prompt sections separated by blank lines and variable parts enclosed in curly braces.).
