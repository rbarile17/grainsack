...
class Evaluate(luigi.Task):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.output_path = EVALUATIONS_PATH / f"{self.kge_model_name}_{self.kg_name}_{lpx_and_eval_config_hash}.json"
...
class Comparison(luigi.WrapperTask):
    ...
    def requires(self):
        ...
        df = pd.read_csv("setup.csv", sep=";")
        return [Metrics(**row) for row in df.to_dict(orient="records")]
...
class Validation(luigi.WrapperTask):
    ...
    def requires(self):
        """Instantiate the DAG from the experimental setup."""
        return [Metrics(kg="FR200K", model="TransE", llm="")]
