...
def estimate_rank_variation(
    kg, kge_model, kge_model_kwargs, fuse, replication_entities, prediction, statements, original_statements, partition, mode
) -> float:
    ...
    mask = (triples[:, [0, 2]].unsqueeze(-1) == replication_entities).any(dim=1)
    ...
    triples = triples[(triples != 0).any(dim=-1)]
    ...
    base_model = kelpie_model_class(kg.training, kge_model, kge_model_kwargs, n_replications, n_statements)
    base_model.cuda()
    ...
    pt_model = kelpie_model_class(kg.training, kge_model, kge_model_kwargs, n_replications, n_statements)
    pt_model = post_train(
...
def post_train(
    kge_model, fuse, prediction, triples, replication_entities, kelpie_entities, statements, original_statements, partition
) -> Model:
    """Post-train the KGE model on the modified KG.

    Modify the KG according to the statement set and post-train the KGE model on the modified KG.
    Post-training is done by creating a new entity embedding for each statement set,
    freezing the existing entity and relation embeddings of the KGE model,
    and training the KGE model on the modified KG.

    Args:
        kge_model: the KGE model to be post-trained.
        modify: the function modifying the KG based on the statements.
        kelpie_kg: the modified KG.
        statements: the statements for modifying the KG.
    """
    ...
    trainer = SLCWATrainingLoop(model=kge_model, triples_factory=post_triples, optimizer=optimizer)
