...
def estimate_rank_variation(kg, model, model_kwargs, fuse, replication_entities, prediction, statements, original_statements, partition) -> float:
    ...
    base_model.cuda()
    base_results = evaluate(base_model, kelpie_prediction.reshape(-1, 3), filter=[triples])
    ...
    pt_results = evaluate(pt_model, kelpie_prediction.reshape(-1, 3), filter=[triples])
