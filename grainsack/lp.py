...
def evaluate(model, triples, filter):
    evaluator = RankBasedEvaluator(clear_on_finalize=False)

    evaluator.evaluate(model, triples, targets=("tail",), additional_filter_triples=filter, use_tqdm=False)
