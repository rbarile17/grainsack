...
class KelpieEmbedding(Embedding):
    ...
    def __init__(self, weight, max_id: int, shape: int, n_statements: int, dtype=torch.float):
        ...
        self._embeddings = NoResetEmbedding(
            weight, num_embeddings=max_id, embedding_dim=self.embedding_dim)
        if self.embedding_dim == 1:
            self._embeddings.weight.data = self._embeddings.weight.data.unsqueeze(
                -1)
        self.kelpie_embeddings = ModuleList(
            [NoResetEmbedding(num_embeddings=1, embedding_dim=self.embedding_dim)
        ...
        for kelpie_embedding in self.kelpie_embeddings:
            if self.is_complex:
                ...
                kelpie_embedding.weight = xavier_uniform_(
                    kelpie_embedding.weight)
    ...
    def _plain_forward(self, indices: LongTensor | None = None) -> FloatTensor:
        ...
        if indices is None:
            ...
            x = torch.zeros(
                (indices.size(0), self.embedding_dim)).to(self.device)
            ...
            if kelpie_indices.numel() > 0:
                ...
                x[mask] = torch.vstack(
                    [self.kelpie_embeddings[i](zero_index) for i in kelpie_indices])
...
class KelpieRelationEmbeddings(Embedding):
    def __init__(self, weight, max_id: int, shape: int):
        ...
        self._embeddings = NoResetEmbedding(
            weight, num_embeddings=max_id, embedding_dim=shape[0])
        self._embeddings.weight.requires_grad = False
...
class KelpieTransE(TransE):
    ...
    def __init__(self, triples_factory, model: Model, config, n_statements):
        ...
        self.entity_representations[0] = KelpieEmbedding(
            weight, max_id, shape, n_statements)
        ...
        self.relation_representations[0] = KelpieRelationEmbeddings(
            weight, max_id, shape)
...
class CustomConvE(ConvE):
    ...
    def criage_first_step(self, triples):
        ...
        s, p, _ = self._get_representations(
            h=triples[..., 0], r=triples[..., 1], t=None, mode=None)
...
class KelpieComplEx(CustomComplEx):
    ...
    def __init__(self, triples_factory, model, config, n_statements):
        ...
        shape = self.entity_representations[0].shape
        self.entity_representations[0] = KelpieEmbedding(
            weight, max_id, shape, n_statements, dtype=torch.cfloat)
...
class KelpieConvE(CustomConvE):
    ...
    def __init__(self, triples_factory, model, config, n_statements):
        ...
        super().__init__(triples_factory=triples_factory, **config,
                         random_seed=42, apply_batch_normalization=False)
        ...
        self.entity_representations[1] = KelpieEmbedding(
            weight, max_id, shape, n_statements)
...
def rank(model, triples, filtr):
    ...
    evaluator.evaluate(model, triples, targets=(
        "tail",), additional_filter_triples=filtr, use_tqdm=False, batch_size=64)
...
def train_kge_model(kge_model, triples, **kge_config):
    optimizer = Adam(params=kge_model.get_grad_params(),
                     **kge_config["optimizer_kwargs"])
    lr_scheduler = ExponentialLR(
        optimizer=optimizer, **kge_config["lr_scheduler_kwargs"])
    ...
    trainer.train(triples_factory=training_triples, **
                  kge_config["training_kwargs"], use_tqdm=False)
