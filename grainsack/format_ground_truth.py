...
def format_fr200k():
    """Format the FR200K dataset for training and testing."""
    ...
    df.to_csv(KGS_PATH / "FR200K" / "data.txt", sep="\t", index=False, header=False)
...
def format_fruni():
    ...
    df.to_csv(KGS_PATH / "FRUNI" / "data.txt", sep="\t", index=False, header=False)
