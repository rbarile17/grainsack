"""This module implements the operation `explain`, the components involved and the function factory for building the explain function from the explanation config."""
...
def run_explain(predictions, kg, kge_model_path, kge_config, lpx_config, build_explainer):
    ...
    :return: The list of explanations each as a tensor of triple.
    :rtype: list
    """
...
def build_combinatorial_optimization_explainer(kg, kge_model_path, kge_config, lpx_config):
    ...
    if method in [KELPIE, DATA_POISONING, CRIAGE]:
        ...
        if method == KELPIE:
            ...
            relevance_partial = partial(estimate_rank_variation, kg, kge_model, kge_config["model_kwargs"], fuse)
        elif method == CRIAGE:
...
def select_replication_entities(kg, kge_model, prediction, k=3):
    """Select the replication entities.

    Select the entities in the KG that:
...
def run_combinatorial_optimization(
    relevance, get_statements, summarize, select_replication_entities_partial, prediction, max_length=4
):
