...
def build_combinatorial_optimization_explainer(kg, kge_model_path, kge_config, lpx_config):
    ...
    if method in [KELPIE, DATA_POISONING, CRIAGE]:
        ...
        if method == KELPIE:
            ...
            relevance_partial = partial(estimate_rank_variation, kg, kge_model, kge_config["model_kwargs"], fuse, mode=mode)
        elif method == CRIAGE:
...
def run_combinatorial_optimization(
    relevance, get_statements, summarize, select_replication_entities_partial, prediction, max_length=4, kelpie=True
):
